    ...
    else
    {
        ...
        while(is >> inputType)
        {
            ...
            else if(inputType == "a") //live ant square
            {
                ...
                if(player == 0)
                    state.myAnts.push_back(Ant(row, col));
                else
...
@param from
    Starting location
@param to
    Vector of possible destinations
@return
    The index of the location in "to" that is closest to "from"
*/
int State::getClosest(Location &from, std::vector<Location> &to)
{
    if (to.size() == 0) return -1;

    int index = 0;
    int closestDist = (from.row - to[0].row)*(from.row - to[0].row) + (from.col - to[0].col)*(from.col - to[0].col);
    for (int i = 1; i < to.size(); i++)
    {
        int dist = (from.row - to[i].row)*(from.row - to[i].row) + (from.col - to[i].col)*(from.col - to[i].col);
...
@param loc
    A Location.
    Can be past the bounds of the grid, but not by more than once the size of the grid.
@return
